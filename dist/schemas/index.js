...
export const GetSchemaInputSchema = z
    ...
    entity_name: z
        ...
        .min(1)
        .optional()
        .describe("Logical name of the Dataverse table (e.g. 'account', 'contact'). " +
        "Omit to list all available tables."),
    include_columns: z
        .boolean()
        .default(true)
        .describe("Include column (attribute) metadata: name, type, required level. " +
        "Only applies when entity_name is specified."),
    include_relationships: z
        .boolean()
        .default(false)
        .describe("Include relationship metadata (1:N, N:1, N:N). " +
        "Only applies when entity_name is specified."),
    response_format: z
...
export const QueryDataInputSchema = z
    ...
    entity_set_name: z
        .string()
        .min(1)
        .describe("EntitySet name of the Dataverse table (e.g. 'accounts', 'contacts', 'leads'). " +
        "Use dataverse_get_schema to find the correct EntitySet name."),
    select: z
        .array(z.string().min(1))
        .optional()
        .describe("List of column logical names to return ($select). " +
        "Omit to return all columns (not recommended for large tables)."),
    filter: z
        .string()
        .optional()
        .describe("OData filter expression ($filter). Examples: " +
        "'statecode eq 0', " +
        "'name eq \\'Contoso\\'', " +
    ...
    order_by: z
        .string()
        .optional()
        .describe("OData sort expression ($orderby). Examples: 'name asc', 'createdon desc', 'name asc,createdon desc'."),
    top: z
        .number()
        .int()
        .min(1)
    ...
    skip_token: z
        .string()
        .optional()
        .describe("Pagination token from a previous response's skip_token field. " +
