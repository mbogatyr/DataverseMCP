...
import { GetSchemaInputSchema, ResponseFormat } from "../schemas/index.js";
...
async function listEntities(client, format) {
    ...
    for (const e of entities) {
        lines.push(`| \`${e.LogicalName}\` | ${getLabel(e.DisplayName)} | \`${e.EntitySetName ?? ""}\` | \`${e.PrimaryIdAttribute ?? ""}\` | ${e.IsCustomEntity ? "Yes" : "No"} |`);
    }
...
async function getEntitySchema(client, entityName, includeColumns, includeRelationships, format) {
    ...
    const params = {
        $select: "MetadataId,LogicalName,SchemaName,DisplayName,DisplayCollectionName,EntitySetName,PrimaryIdAttribute,PrimaryNameAttribute,IsCustomEntity,IsActivity,Description",
    };
...
function appendRelationships(lines, title, rels) {
    ...
    lines.push("|---|---|---|---|---|");
    for (const r of rels) {
        lines.push(`| \`${r.SchemaName}\` | \`${r.ReferencedEntity ?? ""}\` | \`${r.ReferencingEntity ?? ""}\` | \`${r.ReferencedAttribute ?? ""}\` | \`${r.ReferencingAttribute ?? ""}\` |`);
    }
}
function appendManyToMany(lines, rels) {
    if (!rels || rels.length === 0)
        return;
    ...
    lines.push("|---|---|---|");
    for (const r of rels) {
        lines.push(`| \`${r.SchemaName}\` | \`${r.Entity1LogicalName ?? ""}\` | \`${r.Entity2LogicalName ?? ""}\` |`);
    }
}
...
// ─── Tool registration ────────────────────────────────────────────────────────
export function registerGetSchemaTool(server, client) {
    server.registerTool("dataverse_get_schema", {
        ...
        try {
            if (!params.entity_name) {
                ...
                return {
                    ...
                    structuredContent: { count: data.length, entities: data },
                };
            }
            const { text, data } = await getEntitySchema(client, params.entity_name, params.include_columns, params.include_relationships, params.response_format);
            return {
                content: [{ type: "text", text }],
