...
import { QueryDataInputSchema, ResponseFormat, type QueryDataInput } from "../schemas/index.js";
import type { DataverseClient } from "../services/dataverse.js";
import { handleApiError } from "../services/dataverse.js";
import type { ODataCollectionResponse, QueryDataResult } from "../types.js";
...
function buildODataParams(params: QueryDataInput): Record<string, string> {
...
function formatAsMarkdown(
  ...
  if (!firstRecord) return lines.join("\n");

  const columns = Object.keys(firstRecord).filter(
    (k) => !k.startsWith("@") && !k.endsWith("@OData.Community.Display.V1.FormattedValue")
  );
...
Examples:
  ...
  - "Next page" → entity_set_name='accounts', skip_token='<token from previous response>'`,
    ...
    async (params: QueryDataInput) => {
      try {
        const odataParams = buildODataParams(params);

        const response = await client.getCollection<Record<string, unknown>>(
          `/${params.entity_set_name}`,
          odataParams
        );
